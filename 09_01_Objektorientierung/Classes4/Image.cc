...
void Image::save_image(const char *file_name) const
{
    ...
    for (std::uint32_t x = 0; x < m_width; ++x)
    {
        ...
        {
            for (std::int32_t i = (COUNT_RGB - 1); i >= 0; i--)
            {
    ...
    // Header of Bitmap-File with Metadata
    ...
    {
        fwrite(img + (m_width * (m_height - i - 1) * COUNT_RGB), COUNT_RGB, m_width, f);
        fwrite(bmppad, 1, (4 - (m_width * COUNT_RGB) % 4) %4, f);
    }
